...
1. **ZST tokens cost zero bytes** — they are compile-time-only proof objects.
2. **Private constructors = unforgeable** — tokens can only be minted by authorized logic.
3. **Trait hierarchies model RBAC** — clean, inherited permission sets.
4. **Lifetime bounds = revocable** — privileges expire automatically when the session ends.
