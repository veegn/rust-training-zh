...
fn check_limit(temp: Celsius, limit: Celsius) -> bool {
    temp > limit // ✅ same units
}

// temp > voltage // ❌ ERROR: mismatched types
```
...
For advanced dimensional analysis (e.g., automatically knowing that `Watts = Volts × Amperes`), use the [`uom`](https://crates.io/crates/uom) (Units of Measurement) crate.
...
|----------|---------------|
| Sensor readings | ✅ Always |
| Thresholds | ✅ Always |
| API boundaries | ✅ Always |
| Internal helpers | ⚠️ Optional |

## Key Takeaways

1. **Newtypes prevent unit confusion** — `Celsius` and `Rpm` are distinct types.
2. **Zero runtime cost** — newtypes compile down to their inner values (e.g., `f64`).
3. **Macro automation** — quickly stamp out units with standard operations.
4. **`uom` for derived units** — use it for complex physics calculations.
