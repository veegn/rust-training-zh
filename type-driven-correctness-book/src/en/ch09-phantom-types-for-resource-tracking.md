...
|----------|:------:|
| Register widths | ✅ Always |
| DMA direction | ✅ Always |
| File descriptor state | ✅ Always |
| Permissions (R/W/X) | ✅ Always |
| Runtime-variable state | ❌ Use enums |

## Key Takeaways

1. **`PhantomData` markers cost zero bytes** — they are compile-time-only labels.
2. **Structural prevention of mismatches** — register width and DMA direction are enforced by the type system.
3. **Synergy with other patterns** — combine with dimensional types (ch06) for even stronger guarantees.
4. **Compile-time only** — they do not work for attributes that change at runtime.
