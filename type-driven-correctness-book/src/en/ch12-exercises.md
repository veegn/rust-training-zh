...
Model a BMC firmware update lifecycle: `Idle → Uploading → Verifying → Applying → Rebooting → Complete`.
...
Build a complete pipeline: `ADC → calibration → threshold check → result`.
- Use newtypes for `Celsius`, `Volts`, `Watts`.
- Implement `P = V × I` arithmetic in the type system.
...
1. **Practice with realistic protocols** — NVMe and firmware updates are perfect targets for these patterns.
2. **Patterns are composable** — real-world exercises often require combining 2-3 patterns.
3. **Static enforcement is the goal** — if you can represent a protocol violation as a compile error, you've succeeded.
