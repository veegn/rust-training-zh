...
Encode the connection lifecycle into the type system: `Disconnected → Connected → Authenticated → Closed`.
...
Represent the Redfish resource tree as types: `ServiceRoot → ChassisCollection → ChassisInstance → Thermal`.
...
1. **Composition is Power** — combining 5+ patterns creates a "hardened" client where protocol violations are compile errors.
2. **Zero Overhead** — all these checks (tokens, state, phantom types) are erased at compile time.
3. **Formalizing Assumptions** — REST APIs are often loosely defined; types force you to formalize exactly what "Authenticated" or "Valid Thermal Data" means.
