...
1. **Construct, Don't Serialize** — let the type system ensure your JSON matches the schema.
2. **Availability as Proof** — proof tokens replace runtime "is initialized" checks.
3. **The Server is the Source of Truth** — a single server-side bug affects every client. Type-level enforcement on the server is the highest-leverage correctness investment you can make.
